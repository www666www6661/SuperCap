...
bsp_status_t inline bsp_pwm_start(bsp_pwm_channel_t ch) {
  if (bsp_pwm_map[ch].tim == &htim1) {
    ...
    HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_TIMERID_TIMER_A);
    HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_TIMERID_TIMER_B);
    HRTIM1->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_A].CMP1xR = 4000;
    HRTIM1->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_A].CMP3xR = 4000;
  }
  return BSP_OK;
}
