...
bsp_status_t hrtim_pwm_start(uint32_t OutputChannel) {
  if (OutputChannel == HRTIM_TIMERID_TIMER_A) {
    ...
    te xing HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TA2);
 ...
 * @param duty_cycle To caculate the CMP value: xCMP1 = Period/2 * (1 – dutyA),
