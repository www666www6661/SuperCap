...
bsp_status_t bsp_pwm_set_comp(bsp_pwm_channel_t ch, float duty_cycle) {
  ...
  if (bsp_pwm_map[ch].tim == &htim1) {
    ...
    HRTIM_HandleTypeDef *hrtim = (HRTIM_HandleTypeDef *)bsp_pwm_map[ch].tim;
    uint32_t period = __HAL_HRTIM_GET_PERIOD(hrtim, bsp_pwm_map[ch].channel);
    uint16_t pulse = (uint16_t)(duty_cycle * (float)period);
    __HAL_HRTIM_SET_COMPARE(hrtim, bsp_pwm_map[ch].channel, HRTIM_COMPAREUNIT_1,
                            pulse);
  }
...
bsp_status_t bsp_pwm_set_freq(bsp_pwm_channel_t ch, float freq) {
  if (bsp_pwm_map[ch].tim == &htim1) {
    ...
    HRTIM_HandleTypeDef *hrtim = (HRTIM_HandleTypeDef *)bsp_pwm_map[ch].tim;
    uint32_t reload = (uint32_t)(HRTIM_CLK_HZ / freq);
    if (reload > 0) {
      __HAL_HRTIM_SET_PERIOD(hrtim, bsp_pwm_map[ch].channel, reload - 1);
    } else {
      return BSP_ERR;
    }
  }
