...
==============================================================================
  [..] The HRTIM HAL API is split into 2 categories:
    ...
    (#)Waveform functions: These functions allow taking advantage of the HRTIM
       flexibility to produce numerous types of control signal. When a HRTIM
timer operates in waveform mode, all the HRTIM features are accessible without
     ...
     (#)Initialize the HRTIM low level resources by implementing the
        ...
        (##)Connect HRTIM pins to MCU I/Os
            ...
            (+++) Configure these GPIO pins in Alternate Function mode using
                  HAL_GPIO_Init()
        (##)When using DMA to control data transfer (e.g
HAL_HRTIM_SimpleBaseStart_DMA())
            ...
            (+++)Configure the priority and enable the NVIC for the transfer
                 complete interrupt on the DMA channel using
HAL_NVIC_SetPriority() and HAL_NVIC_EnableIRQ()
        (##)In case of using interrupt mode (e.g HAL_HRTIM_SimpleBaseStart_IT())
            (+++)Configure the priority and enable the NVIC for the concerned
                 HRTIM interrupt using HAL_NVIC_SetPriority() and
HAL_NVIC_EnableIRQ()
    ...
    (#)HRTIM timers cannot be used until the high resolution unit is ready. This
       can be checked using HAL_HRTIM_PollForDLLCalibration(): this function
returns HAL_OK if DLL calibration is completed or HAL_TIMEOUT if the DLL
calibration is still going on when timeout given as argument expires. DLL
calibration can also be started in interrupt mode using
HAL_HRTIM_DLLCalibrationStart_IT(). In that case an interrupt is generated when
the DLL calibration is completed. Note that as DLL calibration is executed on a
periodic basis an interrupt will be generated at the end of every DLL
calibration operation (worst case: one interrupt every 14 micro seconds !).
     ...
     (#) Configure HRTIM resources shared by all HRTIM timers
        (##)Burst Mode Controller:
                (+++)HAL_HRTIM_BurstModeConfig(): configures the HRTIM burst
mode controller: operating mode (continuous or one-shot mode), clock (source,
prescaler) , trigger(s), period, idle duration.
        ...
        (##)Fault Conditioning:
                ...
                (+++)HAL_HRTIM_FaultModeCtl(): Enables or disables fault
input(s) circuitry. By default all fault inputs are disabled.
        (##)ADC trigger:
                (+++)HAL_HRTIM_ADCTriggerConfig(): configures the source
triggering the update of the ADC trigger register and the ADC trigger. 4
independent triggers are available to start both the regular and the injected
sequencers of the 2 ADCs
...
(source and outputs on which the delayed protection are applied).
                ...
                (+++)Specifying the HRTIM timer registers update policy (e.g.
pre-load enabling).
...
period a defined time period
                (+++)Windowing, to enable external events only during a defined
time period
...
HAL_HRTIM_WaveformOutputConfig()).
              ...
              (++)HAL_HRTIM_WaveformCaptureConfig(): configures the capture unit
                  ...
                  source(s)  triggering the capture (timer register update
event, external event, timer output set/reset event, other HRTIM timer related
events).
              (++)HAL_HRTIM_WaveformOutputConfig(): configuration of a HRTIM
timer output mainly consists in:
                (+++)Setting the output polarity (active high or active low),
                (+++)Defining the set/reset crossbar for the output,
                (+++)Specifying the fault level (active or inactive) in IDLE and
FAULT states.,
     ...
     (#) Set waveform timer output(s) level
              (++)HAL_HRTIM_WaveformSetOutputLevel(): forces the output to its
                  ...
                  is enabled it is necessary to force the output level by
software to have the outputs in a complementary state as soon as the RUN mode is
entered.
     ...
     (#) Some functions can be used any time to retrieve HRTIM timer related
            information
              ...
              (++)HAL_HRTIM_GetCurrentPushPullStatus(): when the push-pull mode
                 is enabled for the HRTIM timer (see
HAL_HRTIM_WaveformTimerConfig()), the push-pull status indicates on which output
the signal is currently active (e.g signal applied on output 1 and output 2
forced inactive or vice versa).
             (++)HAL_HRTIM_GetIdlePushPullStatus(): when the push-pull mode
                 is enabled for the HRTIM timer (see
HAL_HRTIM_WaveformTimerConfig()), the idle push-pull status indicates during
which period the delayed protection request occurred (e.g. protection occurred
     ...
     *** Callback registration ***
     ...
     Use Functions HAL_HRTIM_RegisterCallback() or
HAL_HRTIM_TIMxRegisterCallback() to register an interrupt callback.

     [..]
     Function HAL_HRTIM_RegisterCallback() allows to register following
callbacks:
       ...
       (+) SystemFaultCallback          : System fault interrupt callback
function
       (+) DLLCalibrationReadyCallback  : DLL Ready interrupt callback function
       (+) BurstModePeriodCallback      : Burst mode period interrupt callback
function
     ...
     [..]
     Function HAL_HRTIM_TIMxRegisterCallback() allows to register following
callbacks:
       (+) RegistersUpdateCallback   : Timer x Update interrupt callback
function
       (+) RepetitionEventCallback   : Timer x Repetition interrupt callback
function
       (+) Compare1EventCallback     : Timer x Compare 1 match interrupt
callback function
       (+) Compare2EventCallback     : Timer x Compare 2 match interrupt
callback function
       (+) Compare3EventCallback     : Timer x Compare 3 match interrupt
callback function
       (+) Compare4EventCallback     : Timer x Compare 4 match interrupt
callback function
       (+) Capture1EventCallback     : Timer x Capture 1 interrupts callback
function
       (+) Capture2EventCallback     : Timer x Capture 2 interrupts callback
function
       (+) DelayedProtectionCallback : Timer x Delayed protection interrupt
callback function
       (+) CounterResetCallback      : Timer x counter reset/roll-over interrupt
callback function
       (+) Output1SetCallback        : Timer x output 1 set interrupt callback
function
       (+) Output1ResetCallback      : Timer x output 1 reset interrupt callback
function
       (+) Output2SetCallback        : Timer x output 2 set interrupt callback
function
       (+) Output2ResetCallback      : Timer x output 2 reset interrupt callback
function
       (+) BurstDMATransferCallback  : Timer x Burst DMA completed interrupt
callback function

     [..]
     Both functions take as parameters the HAL peripheral handle, the Callback
ID and a pointer to the user callback function.

     [..]
     Use function HAL_HRTIM_UnRegisterCallback or
HAL_HRTIM_TIMxUnRegisterCallback to reset a callback to the default weak
function. Both functions take  as parameters the HAL peripheral handle and the
Callback ID.

     [..]
     By default, after the HAL_HRTIM_Init() and when the state is
HAL_HRTIM_STATE_RESET all callbacks are set to the corresponding weak functions
(e.g HAL_HRTIM_Fault1Callback) Exception done for MspInit and MspDeInit
functions that are reset to the legacy weak functions in the HAL_HRTIM_Init()/
HAL_HRTIM_DeInit() only when these callbacks are null (not registered
beforehand). If MspInit or MspDeInit are not null, the HAL_HRTIM_Init()/
HAL_HRTIM_DeInit() keep and use the user MspInit/MspDeInit callbacks (registered
beforehand) whatever the state.

     [..]
     Callbacks can be registered/unregistered in HAL_HRTIM_STATE_READY state
only. Exception done MspInit/MspDeInit functions that can be
registered/unregistered in HAL_HRTIM_STATE_READY or HAL_HRTIM_STATE_RESET
states, thus registered (user) MspInit/DeInit callbacks can be used during the
...
#define HRTIM_FLTR_FLTxEN                                                      \
  (HRTIM_FLTR_FLT1EN | HRTIM_FLTR_FLT2EN | HRTIM_FLTR_FLT3EN |                 \
   HRTIM_FLTR_FLT4EN | HRTIM_FLTR_FLT5EN)
...
#define HRTIM_FLTINR1_FLTxLCK                                                  \
  ((HRTIM_FAULTLOCK_READONLY) | (HRTIM_FAULTLOCK_READONLY << 8U) |             \
   (HRTIM_FAULTLOCK_READONLY << 16U) | (HRTIM_FAULTLOCK_READONLY << 24U))
 ...
 */
static void
HRTIM_MasterBase_Config(HRTIM_HandleTypeDef *hhrtim,
                        const HRTIM_TimeBaseCfgTypeDef *pTimeBaseCfg);

static void
HRTIM_TimingUnitBase_Config(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
                            const HRTIM_TimeBaseCfgTypeDef *pTimeBaseCfg);
...
static void
HRTIM_TimingUnitWaveform_Config(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
                                const HRTIM_TimerCfgTypeDef *pTimerCfg);

static void HRTIM_CaptureUnitConfig(HRTIM_HandleTypeDef *hhrtim,
                                    uint32_t TimerIdx, uint32_t CaptureUnit,
                                    uint32_t Event);

static void HRTIM_OutputConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
                               uint32_t Output,
                               const HRTIM_OutputCfgTypeDef *pOutputCfg);

static void HRTIM_EventConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t Event,
                              const HRTIM_EventCfgTypeDef *pEventCfg);

static void HRTIM_TIM_ResetConfig(HRTIM_HandleTypeDef *hhrtim,
                                  uint32_t TimerIdx, uint32_t Event);

static uint32_t HRTIM_GetITFromOCMode(const HRTIM_HandleTypeDef *hhrtim,
                                      uint32_t TimerIdx, uint32_t OCChannel);

static uint32_t HRTIM_GetDMAFromOCMode(const HRTIM_HandleTypeDef *hhrtim,
                                       uint32_t TimerIdx, uint32_t OCChannel);

static DMA_HandleTypeDef *
HRTIM_GetDMAHandleFromTimerIdx(const HRTIM_HandleTypeDef *hhrtim,
                               uint32_t TimerIdx);
...
/** @defgroup HRTIM_Exported_Functions_Group1 Initialization and
...
HAL_StatusTypeDef HAL_HRTIM_Init(HRTIM_HandleTypeDef *hhrtim) {
  ...
  if (hhrtim->State == HAL_HRTIM_STATE_RESET) {
    ...
    hhrtim->SynchronizationEventCallback =
  ...
  for (timer_idx = HRTIM_TIMERINDEX_TIMER_A;
       timer_idx <= HRTIM_TIMERINDEX_MASTER; timer_idx++) {
 ...
 *                    @arg HRTIM_CALIBRATIONRATE_7300: Periodic DLL calibration.
 * T=7.300 ms
 *                    @arg HRTIM_CALIBRATIONRATE_910:  Periodic DLL calibration.
 * T=0.910 ms
 *                    @arg HRTIM_CALIBRATIONRATE_114:  Periodic DLL calibration.
 * T=0.114 ms
 *                    @arg HRTIM_CALIBRATIONRATE_14:   Periodic DLL calibration.
 * T=0.014 ms
 * @retval HAL status
...
HAL_StatusTypeDef HAL_HRTIM_DLLCalibrationStart(HRTIM_HandleTypeDef *hhrtim,
  ...
  if (CalibrationRate == HRTIM_SINGLE_CALIBRATION) {
    ...
    MODIFY_REG(hhrtim->Instance->sCommonRegs.DLLCR, HRTIM_DLLCR_CALRTE,
               CalibrationRate);
    SET_BIT(hhrtim->Instance->sCommonRegs.DLLCR, HRTIM_DLLCR_CAL);
  }
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_TimeBaseConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
                         const HRTIM_TimeBaseCfgTypeDef *pTimeBaseCfg) {
  /* Check the parameters */
...
/** @defgroup HRTIM_Exported_Functions_Group3 Simple output compare mode
functions
 ...
 * @param  pSimpleOCChannelCfg pointer to the simple output compare output
 ...
 *       Output Set/Reset crossbar is set according to the selected output
 * compare mode: Toggle: SETxyR = RSTxyR = CMPy Active: SETxyR = CMPy, RSTxyR =
 * 0 Inactive: SETxy =0, RSTxy = CMPy
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HRTIM_SimpleOCChannelConfig(
    HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx, uint32_t OCChannel,
  ...
  HRTIM_OutputConfig(hhrtim, TimerIdx, OCChannel, &OutputCfg);
...
/**
 * @brief  Start the output compare signal generation on the designed timer
 * output
 * @param  hhrtim pointer to HAL HRTIM handle
 * @param  TimerIdx Timer index
 *                   This parameter can be one of the following values:
...
/**
 * @brief  Stop the output compare signal generation on the designed timer
 * output
 * @param  hhrtim pointer to HAL HRTIM handle
 * @param  TimerIdx Timer index
 *                   This parameter can be one of the following values:
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_SimpleOCStart_DMA(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
                            uint32_t OCChannel, uint32_t SrcAddr,
...
HAL_StatusTypeDef HAL_HRTIM_SimpleOCStop_DMA(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* Get the timer DMA handler */
  /* Disable the DMA */
  if (HAL_DMA_Abort(HRTIM_GetDMAHandleFromTimerIdx(hhrtim, TimerIdx)) !=
      HAL_OK) {
    hhrtim->State = HAL_HRTIM_STATE_ERROR;
 ...
 *                    @arg HRTIM_OUTPUT_TE1: Timer E - Output 1
 *                    @arg HRTIM_OUTPUT_TE2: Timer E - Output 2
 * @param  pSimplePWMChannelCfg pointer to the simple PWM output configuration
 * structure
...
HAL_StatusTypeDef HAL_HRTIM_SimplePWMChannelConfig(
    HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx, uint32_t PWMChannel,
  ...
  case HRTIM_OUTPUT_TD2:
  case HRTIM_OUTPUT_TE2: {
    hhrtim->Instance->sTimerxRegs[TimerIdx].CMP2xR =
        pSimplePWMChannelCfg->Pulse;
    OutputCfg.SetSource = HRTIM_OUTPUTSET_TIMCMP2;
    break;
  }
  ...
  HRTIM_OutputConfig(hhrtim, TimerIdx, PWMChannel, &OutputCfg);
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_SimplePWMStart_DMA(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
                             uint32_t PWMChannel, uint32_t SrcAddr,
                             uint32_t DestAddr, uint32_t Length) {
  DMA_HandleTypeDef *hdma;
 ...
 * @param  pSimpleCaptureChannelCfg pointer to the simple capture configuration
 * structure
 * @note When the timer operates in simple capture mode the capture is triggered
 *       by the designated external event and GPIO input is implicitly used as
 * event source. The cature can be triggered by a rising edge, a falling edge or
 * both edges on event channel.
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HRTIM_SimpleCaptureChannelConfig(
    HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx, uint32_t CaptureChannel,
  ...
  assert_param(
      IS_HRTIM_EVENTPOLARITY(pSimpleCaptureChannelCfg->EventSensitivity,
                             pSimpleCaptureChannelCfg->EventPolarity));
  assert_param(
      IS_HRTIM_EVENTSENSITIVITY(pSimpleCaptureChannelCfg->EventSensitivity));
  ...
  EventCfg.Polarity =
      (pSimpleCaptureChannelCfg->EventPolarity & HRTIM_EECR1_EE1POL);
  EventCfg.Sensitivity =
      (pSimpleCaptureChannelCfg->EventSensitivity & HRTIM_EECR1_EE1SNS);
  EventCfg.Source = HRTIM_EVENTSRC_1;

  HRTIM_EventConfig(hhrtim, pSimpleCaptureChannelCfg->Event, &EventCfg);

  /* Memorize capture trigger (will be configured when the capture is started */
  HRTIM_CaptureUnitConfig(hhrtim, TimerIdx, CaptureChannel,
...
HAL_StatusTypeDef HAL_HRTIM_SimpleCaptureStart(HRTIM_HandleTypeDef *hhrtim,
  ...
  case HRTIM_CAPTUREUNIT_1: {
    hhrtim->Instance->sTimerxRegs[TimerIdx].CPT1xCR =
        hhrtim->TimerParam[TimerIdx].CaptureTrigger1;
    break;
  }

  case HRTIM_CAPTUREUNIT_2: {
    hhrtim->Instance->sTimerxRegs[TimerIdx].CPT2xCR =
        hhrtim->TimerParam[TimerIdx].CaptureTrigger2;
    break;
  }
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_SimpleCaptureStart_DMA(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
                                 uint32_t CaptureChannel, uint32_t SrcAddr,
                                 uint32_t DestAddr, uint32_t Length) {
  DMA_HandleTypeDef *hdma;
 ...
 *                    @arg HRTIM_OUTPUT_TE1: Timer E - Output 1
 *                    @arg HRTIM_OUTPUT_TE2: Timer E - Output 2
 * @param  pSimpleOnePulseChannelCfg pointer to the simple one pulse output
 * configuration structure
 * @note When the timer operates in simple one pulse mode:
 *         the timer counter is implicitly started by the reset event,
 *         the reset of the timer counter is triggered by the designated
 * external event GPIO input is implicitly used as event source, Output 1 is
 * implicitly controlled by the compare unit 1, Output 2 is implicitly
 * controlled by the compare unit 2. Output Set/Reset crossbar is set as
 * follows: Output 1: SETx1R = CMP1, RSTx1R = PER Output 2: SETx2R = CMP2, RST2R
 * = PER
 * @retval HAL status
 ...
 */
HAL_StatusTypeDef HAL_HRTIM_SimpleOnePulseChannelConfig(
    HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx, uint32_t OnePulseChannel,
  ...
  assert_param(
      IS_HRTIM_OUTPUTPOLARITY(pSimpleOnePulseChannelCfg->OutputPolarity));
  assert_param(
      IS_HRTIM_OUTPUTIDLELEVEL(pSimpleOnePulseChannelCfg->OutputIdleLevel));
  assert_param(IS_HRTIM_EVENT(pSimpleOnePulseChannelCfg->Event));
  assert_param(
      IS_HRTIM_EVENTPOLARITY(pSimpleOnePulseChannelCfg->EventSensitivity,
                             pSimpleOnePulseChannelCfg->EventPolarity));
  assert_param(
      IS_HRTIM_EVENTSENSITIVITY(pSimpleOnePulseChannelCfg->EventSensitivity));
  ...
  case HRTIM_OUTPUT_TE1: {
    hhrtim->Instance->sTimerxRegs[TimerIdx].CMP1xR =
        pSimpleOnePulseChannelCfg->Pulse;
    OutputCfg.SetSource = HRTIM_OUTPUTSET_TIMCMP1;
    break;
  }
  ...
  /* Configure timer output */
  OutputCfg.Polarity =
      (pSimpleOnePulseChannelCfg->OutputPolarity & HRTIM_OUTR_POL1);
  OutputCfg.IdleLevel =
      (pSimpleOnePulseChannelCfg->OutputIdleLevel & HRTIM_OUTR_IDLES1);
  OutputCfg.FaultLevel = HRTIM_OUTPUTFAULTLEVEL_NONE;
  OutputCfg.IdleMode = HRTIM_OUTPUTIDLEMODE_NONE;
  OutputCfg.ChopperModeEnable = HRTIM_OUTPUTCHOPPERMODE_DISABLED;
  OutputCfg.BurstModeEntryDelayed = HRTIM_OUTPUTBURSTMODEENTRY_REGULAR;
  ...
  HRTIM_OutputConfig(hhrtim, TimerIdx, OnePulseChannel, &OutputCfg);

  /* Configure external event channel */
  EventCfg.FastMode = HRTIM_EVENTFASTMODE_DISABLE;
  EventCfg.Filter = (pSimpleOnePulseChannelCfg->EventFilter & HRTIM_EECR3_EE6F);
  EventCfg.Polarity =
      (pSimpleOnePulseChannelCfg->EventPolarity & HRTIM_OUTR_POL1);
  EventCfg.Sensitivity =
      (pSimpleOnePulseChannelCfg->EventSensitivity & HRTIM_EECR1_EE1SNS);
  EventCfg.Source = HRTIM_EVENTSRC_1;

  HRTIM_EventConfig(hhrtim, pSimpleOnePulseChannelCfg->Event, &EventCfg);

  /* Configure the timer reset register */
  HRTIM_TIM_ResetConfig(hhrtim, TimerIdx, pSimpleOnePulseChannelCfg->Event);
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_BurstModeConfig(HRTIM_HandleTypeDef *hhrtim,
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_EventConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t Event,
                      const HRTIM_EventCfgTypeDef *pEventCfg) {
  /* Check parameters */
  assert_param(IS_HRTIM_EVENT(Event));
  assert_param(IS_HRTIM_EVENTSRC(pEventCfg->Source));
  assert_param(
      IS_HRTIM_EVENTPOLARITY(pEventCfg->Sensitivity, pEventCfg->Polarity));
...
HAL_StatusTypeDef HAL_HRTIM_EventPrescalerConfig(HRTIM_HandleTypeDef *hhrtim,
  ...
  MODIFY_REG(hhrtim->Instance->sCommonRegs.EECR3, HRTIM_EECR3_EEVSD,
             (Prescaler & HRTIM_EECR3_EEVSD));
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_FaultConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t Fault,
  ...
  hhrtim->Instance->sCommonRegs.FLTINR1 =
      (hrtim_fltinr1 & (~(HRTIM_FLTINR1_FLTxLCK)));
  hhrtim->Instance->sCommonRegs.FLTINR2 =
      (hrtim_fltinr2 & (~(HRTIM_FLTINR2_FLTxLCK)));

  /* Update the HRTIM registers LOCK bit */
  SET_BIT(hhrtim->Instance->sCommonRegs.FLTINR1,
          (hrtim_fltinr1 & HRTIM_FLTINR1_FLTxLCK));
  SET_BIT(hhrtim->Instance->sCommonRegs.FLTINR2,
          (hrtim_fltinr2 & HRTIM_FLTINR2_FLTxLCK));
...
HAL_StatusTypeDef HAL_HRTIM_FaultPrescalerConfig(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* Set the external event prescaler */
  MODIFY_REG(hhrtim->Instance->sCommonRegs.FLTINR2, HRTIM_FLTINR2_FLTSD,
             (Prescaler & HRTIM_FLTINR2_FLTSD));
 ...
 * @retval None
 */
void HAL_HRTIM_FaultModeCtl(HRTIM_HandleTypeDef *hhrtim, uint32_t Faults,
                            uint32_t Enable) {
  /* Check parameters */
  ...
  if ((Faults & HRTIM_FAULT_1) != (uint32_t)RESET) {
    MODIFY_REG(hhrtim->Instance->sCommonRegs.FLTINR1, HRTIM_FLTINR1_FLT1E,
               (Enable & HRTIM_FLTINR1_FLT1E));
  }
  ...
  if ((Faults & HRTIM_FAULT_5) != (uint32_t)RESET) {
    MODIFY_REG(hhrtim->Instance->sCommonRegs.FLTINR2, HRTIM_FLTINR2_FLT5E,
               ((Enable)&HRTIM_FLTINR2_FLT5E));
  }
}
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_ADCTriggerConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t ADCTrigger,
...
/** @defgroup HRTIM_Exported_Functions_Group8 Timer waveform configuration and
functions
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_WaveformTimerConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
                              const HRTIM_TimerCfgTypeDef *pTimerCfg) {
  /* Check parameters */
  assert_param(IS_HRTIM_TIMERINDEX(TimerIdx));
  ...
  if (TimerIdx == HRTIM_TIMERINDEX_MASTER) {
    ...
    assert_param(IS_HRTIM_TIMDELAYEDPROTECTION(
        pTimerCfg->PushPull, pTimerCfg->DelayedProtectionMode));
...
/**
 * @brief  Configure the event filtering capabilities of a timer (blanking,
 * windowing)
 * @param  hhrtim pointer to HAL HRTIM handle
 * @param  TimerIdx Timer index
 *                   This parameter can be one of the following values:
 ...
 *                   @arg HRTIM_TIMERINDEX_TIMER_D for timer D
 *                   @arg HRTIM_TIMERINDEX_TIMER_E for timer E
 * @param  Event external event for which timer event filtering must be
 ...
 *                    @arg HRTIM_EVENT_9: External event 9
 *                    @arg HRTIM_EVENT_10: External event 10
 * @param  pTimerEventFilteringCfg pointer to the timer event filtering
 * configuration structure
 * @note This function must be called before starting the timer
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HRTIM_TimerEventFilteringConfig(
    HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx, uint32_t Event,
  ...
  case HRTIM_EVENT_1: {
    MODIFY_REG(
        hhrtim->Instance->sTimerxRegs[TimerIdx].EEFxR1,
  ...
  case HRTIM_EVENT_6: {
    MODIFY_REG(
        hhrtim->Instance->sTimerxRegs[TimerIdx].EEFxR2,
 ...
 *                   @arg HRTIM_TIMERINDEX_TIMER_D for timer D
 *                   @arg HRTIM_TIMERINDEX_TIMER_E for timer E
 * @param  pDeadTimeCfg pointer to the deadtime insertion configuration
 * structure
 * @retval HAL status
 * @note This function must be called before starting the timer
 */
HAL_StatusTypeDef
HAL_HRTIM_DeadTimeConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
  ...
  assert_param(
      IS_HRTIM_TIMDEADTIME_RISINGSIGNLOCK(pDeadTimeCfg->RisingSignLock));
  assert_param(IS_HRTIM_TIMDEADTIME_FALLINGSIGN(pDeadTimeCfg->FallingSign));
  assert_param(IS_HRTIM_TIMDEADTIME_FALLINGLOCK(pDeadTimeCfg->FallingLock));
  assert_param(
      IS_HRTIM_TIMDEADTIME_FALLINGSIGNLOCK(pDeadTimeCfg->FallingSignLock));
 ...
 * @note When auto delayed mode is required for compare unit 2 or compare unit
 * 4, application has to configure separately the capture unit. Capture unit to
 * configure in that case depends on the compare unit auto delayed mode is
 * applied to (see below): Auto delayed on output compare 2: capture unit 1 must
 * be configured Auto delayed on output compare 4: capture unit 2 must be
 * configured
 * @retval HAL status
 * @note This function must be called before starting the timer
 */
HAL_StatusTypeDef
HAL_HRTIM_WaveformCompareConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
  ...
  if (TimerIdx == HRTIM_TIMERINDEX_MASTER) {
    ...
    case HRTIM_COMPAREUNIT_2: {
      /* Check parameters */
      assert_param(IS_HRTIM_COMPAREUNIT_AUTODELAYEDMODE(
          CompareUnit, pCompareCfg->AutoDelayedMode));
    ...
    case HRTIM_COMPAREUNIT_4: {
      ...
      /* Set the compare value */
      hhrtim->Instance->sTimerxRegs[TimerIdx].CMP4xR =
          pCompareCfg->CompareValue;
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_WaveformCaptureConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
  ...
  switch (CaptureUnit) {
  case HRTIM_CAPTUREUNIT_1: {
    WRITE_REG(hhrtim->Instance->sTimerxRegs[TimerIdx].CPT1xCR,
              pCaptureCfg->Trigger);
    break;
  }

  case HRTIM_CAPTUREUNIT_2: {
    WRITE_REG(hhrtim->Instance->sTimerxRegs[TimerIdx].CPT2xCR,
              pCaptureCfg->Trigger);
    break;
  }
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_WaveformOutputConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
                               uint32_t Output,
                               const HRTIM_OutputCfgTypeDef *pOutputCfg) {
  /* Check parameters */
  ...
  assert_param(
      IS_HRTIM_OUTPUTBURSTMODEENTRY(pOutputCfg->BurstModeEntryDelayed));
  ...
  HRTIM_OutputConfig(hhrtim, TimerIdx, Output, pOutputCfg);
 ...
 *                    @arg HRTIM_OUTPUT_TE1: Timer E - Output 1
 *                    @arg HRTIM_OUTPUT_TE2: Timer E - Output 2
 * @param OutputLevel indicates whether the output is forced to its active or
 * inactive level This parameter can be one of the following values:
 *                    @arg HRTIM_OUTPUTLEVEL_ACTIVE: output is forced to its
 * active level
 *                    @arg HRTIM_OUTPUTLEVEL_INACTIVE: output is forced to its
 * inactive level
 * @retval HAL status
...
/**
 * @brief  Enable the generation of the waveform signal on the designated
 * output(s) Outputs can be combined (ORed) to allow for simultaneous output
 * enabling.
 * @param  hhrtim pointer to HAL HRTIM handle
...
/**
 * @brief  Disable the generation of the waveform signal on the designated
 * output(s) Outputs can be combined (ORed) to allow for simultaneous output
 * disabling.
 * @param  hhrtim pointer to HAL HRTIM handle
...
/**
 * @brief  Start the counter of the designated timer(s) operating in waveform
 * mode Timers can be combined (ORed) to allow for simultaneous counter start.
 * @param  hhrtim pointer to HAL HRTIM handle
...
/**
 * @brief  Stop the counter of the designated timer(s) operating in waveform
 * mode Timers can be combined (ORed) to allow for simultaneous counter stop.
 * @param  hhrtim pointer to HAL HRTIM handle
 ...
 *                   @arg HRTIM_TIMERID_TIMER_E
 * @retval HAL status
 * @note The counter of a timer is stopped only if all timer outputs are
 * disabled
 */
...
HAL_StatusTypeDef HAL_HRTIM_WaveformCountStart_IT(HRTIM_HandleTypeDef *hhrtim,
  ...
  if ((Timers & HRTIM_TIMERID_MASTER) != 0U) {
    __HAL_HRTIM_MASTER_ENABLE_IT(
        hhrtim, hhrtim->TimerParam[HRTIM_TIMERINDEX_MASTER].InterruptRequests);
  }
  ...
  for (timer_idx = HRTIM_TIMERINDEX_TIMER_A;
    ...
    if ((Timers & TimerIdxToTimerId[timer_idx]) != 0U) {
      __HAL_HRTIM_TIMER_ENABLE_IT(
          hhrtim, timer_idx, hhrtim->TimerParam[timer_idx].InterruptRequests);
    }
  }
...
HAL_StatusTypeDef HAL_HRTIM_WaveformCountStart_DMA(HRTIM_HandleTypeDef *hhrtim,
  ...
  if (((Timers & HRTIM_TIMERID_MASTER) != (uint32_t)RESET) &&
    ...
    __HAL_HRTIM_MASTER_ENABLE_DMA(
        hhrtim, hhrtim->TimerParam[HRTIM_TIMERINDEX_MASTER].DMARequests);
  }
  ...
  for (timer_idx = HRTIM_TIMERINDEX_TIMER_A;
    ...
    if (((Timers & TimerIdxToTimerId[timer_idx]) != (uint32_t)RESET) &&
      ...
      /* Enable the timer DMA request */
      __HAL_HRTIM_TIMER_ENABLE_DMA(hhrtim, timer_idx,
...
HAL_StatusTypeDef HAL_HRTIM_WaveformCountStop_DMA(HRTIM_HandleTypeDef *hhrtim,
  ...
  for (timer_idx = HRTIM_TIMERINDEX_TIMER_A;
    ...
    if (((Timers & TimerIdxToTimerId[timer_idx]) != 0U) &&
      ...
      /* Get the timer DMA handler */
      /* Disable the DMA */
      if (HAL_DMA_Abort(HRTIM_GetDMAHandleFromTimerIdx(hhrtim, timer_idx)) !=
          HAL_OK) {
        hhrtim->State = HAL_HRTIM_STATE_ERROR;
      } else {
        hhrtim->State = HAL_HRTIM_STATE_READY;
        ...
        __HAL_HRTIM_TIMER_DISABLE_DMA(
            hhrtim, timer_idx, hhrtim->TimerParam[timer_idx].DMARequests);
      }
    }
  }
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_BurstModeSoftwareTrigger(HRTIM_HandleTypeDef *hhrtim) {
  if (hhrtim->State == HAL_HRTIM_STATE_BUSY) {
    return HAL_BUSY;
  }
...
HAL_StatusTypeDef HAL_HRTIM_SoftwareCapture(HRTIM_HandleTypeDef *hhrtim,
  ...
  switch (CaptureUnit) {
  case HRTIM_CAPTUREUNIT_1: {
    SET_BIT(hhrtim->Instance->sTimerxRegs[TimerIdx].CPT1xCR,
            HRTIM_CPT1CR_SWCPT);
    break;
  }

  case HRTIM_CAPTUREUNIT_2: {
    SET_BIT(hhrtim->Instance->sTimerxRegs[TimerIdx].CPT2xCR,
            HRTIM_CPT2CR_SWCPT);
    break;
  }
 ...
 *       HRTIM_TIMERINDEX_MASTER: DMA channel 2 is used by the DMA burst
 * controller HRTIM_TIMERINDEX_TIMER_A: DMA channel 3 is used by the DMA burst
 * controller HRTIM_TIMERINDEX_TIMER_B: DMA channel 4 is used by the DMA burst
 * controller HRTIM_TIMERINDEX_TIMER_C: DMA channel 5 is used by the DMA burst
 * controller HRTIM_TIMERINDEX_TIMER_D: DMA channel 6 is used by the DMA burst
 * controller HRTIM_TIMERINDEX_TIMER_E: DMA channel 7 is used by the DMA burst
 * controller
 */
HAL_StatusTypeDef HAL_HRTIM_BurstDMATransfer(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* Enable the DMA channel */
  if (HAL_DMA_Start_IT(hdma, BurstBufferAddress,
...
/**
 * @brief  Return actual value of the capture register of the designated capture
 * unit
 * @param  hhrtim pointer to HAL HRTIM handle
 * @param  TimerIdx Timer index
 *                   This parameter can be one of the following values:
...
uint32_t HAL_HRTIM_WaveformGetOutputLevel(const HRTIM_HandleTypeDef *hhrtim,
                                          uint32_t TimerIdx, uint32_t Output) {
  ...
  case HRTIM_OUTPUT_TD1:
  case HRTIM_OUTPUT_TE1: {
    if ((hhrtim->Instance->sTimerxRegs[TimerIdx].TIMxISR &
         HRTIM_TIMISR_O1CPY) != (uint32_t)RESET) {
  ...
  case HRTIM_OUTPUT_TE2: {
    if ((hhrtim->Instance->sTimerxRegs[TimerIdx].TIMxISR &
         HRTIM_TIMISR_O2CPY) != (uint32_t)RESET) {
      output_level = HRTIM_OUTPUTLEVEL_ACTIVE;
    } else {
      output_level = HRTIM_OUTPUTLEVEL_INACTIVE;
    }
...
uint32_t HAL_HRTIM_WaveformGetOutputState(const HRTIM_HandleTypeDef *hhrtim,
  ...
  /* Set output state according to output control status and output disable
   * status */
  switch (Output) {
  case HRTIM_OUTPUT_TA1: {
  ...
  if ((hhrtim->Instance->sCommonRegs.OENR & output_bit) != (uint32_t)RESET) {
    /* Output is enabled: output in RUN state (whatever output disable status
     * is)*/
...
uint32_t HAL_HRTIM_GetDelayedProtectionStatus(const HRTIM_HandleTypeDef *hhrtim,
  ...
  case HRTIM_OUTPUT_TE1: {
    if ((hhrtim->Instance->sTimerxRegs[TimerIdx].TIMxISR &
         HRTIM_TIMISR_O1STAT) != (uint32_t)RESET) {
  ...
  case HRTIM_OUTPUT_TE2: {
    if ((hhrtim->Instance->sTimerxRegs[TimerIdx].TIMxISR &
         HRTIM_TIMISR_O2STAT) != (uint32_t)RESET) {
...
/**
 * @brief  Return the actual status (active or inactive) of the burst mode
 * controller
 * @param  hhrtim pointer to HAL HRTIM handle
...
uint32_t HAL_HRTIM_GetCurrentPushPullStatus(const HRTIM_HandleTypeDef *hhrtim,
  ...
  current_pushpull_status =
      (hhrtim->Instance->sTimerxRegs[TimerIdx].TIMxISR & HRTIM_TIMISR_CPPSTAT);
...
/**
  * @brief  Indicate on which output the signal was applied, in push-pull mode,
            balanced fault mode or delayed idle mode, when the protection was
  triggered.
  * @param  hhrtim pointer to HAL HRTIM handle
  * @param  TimerIdx Timer index
  *                   This parameter can be one of the following values:
...
uint32_t HAL_HRTIM_GetIdlePushPullStatus(const HRTIM_HandleTypeDef *hhrtim,
  ...
  idle_pushpull_status =
      (hhrtim->Instance->sTimerxRegs[TimerIdx].TIMxISR & HRTIM_TIMISR_IPPSTAT);
 ...
 *            - System fault and Fault 1 to 5 (regardless of the timing unit
attribution)
...
@verbatim
 ...
 ===============================================================================
    ...
    interrupts:
      ...
      (+)  Callback function called when a timer register update interrupt
occurs
...
__weak void HAL_HRTIM_SystemFaultCallback(HRTIM_HandleTypeDef *hhrtim) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_SystemFaultCallback could be implemented in the user
     file
   */
}
...
__weak void HAL_HRTIM_DLLCalibrationReadyCallback(HRTIM_HandleTypeDef *hhrtim) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_DLLCalibrationCallback could be implemented in the
     user file
   */
}

/**
 * @brief  Callback function invoked when the end of the burst mode period is
 * reached
 * @param  hhrtim pointer to HAL HRTIM handle
 * @retval None
 */
__weak void HAL_HRTIM_BurstModePeriodCallback(HRTIM_HandleTypeDef *hhrtim) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_BurstModeCallback could be implemented in the user
     file
   */
}

/**
 * @brief  Callback function invoked when a synchronization input event is
 * received
 * @param  hhrtim pointer to HAL HRTIM handle
 * @retval None
 */
__weak void
HAL_HRTIM_SynchronizationEventCallback(HRTIM_HandleTypeDef *hhrtim) {
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);

  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_SynchronizationEventCallback could be implemented in
     the user file
   */
}
...
__weak void HAL_HRTIM_RegistersUpdateCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Master_RegistersUpdateCallback could be implemented in
     the user file
   */
}
...
__weak void HAL_HRTIM_RepetitionEventCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Master_RepetitionEventCallback could be implemented in
     the user file
   */
}
...
__weak void HAL_HRTIM_Compare1EventCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Master_Compare1EventCallback could be implemented in
     the user file
   */
}
...
__weak void HAL_HRTIM_Compare2EventCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Master_Compare2EventCallback could be implemented in
     the user file
   */
}
...
__weak void HAL_HRTIM_Compare3EventCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Master_Compare3EventCallback could be implemented in
     the user file
   */
}
...
__weak void HAL_HRTIM_Compare4EventCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Master_Compare4EventCallback could be implemented in
     the user file
   */
}
...
__weak void HAL_HRTIM_Capture1EventCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Timer_Capture1EventCallback could be implemented in
     the user file
   */
}
...
__weak void HAL_HRTIM_Capture2EventCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Timer_Capture2EventCallback could be implemented in
     the user file
   */
}

/**
 * @brief  Callback function invoked when the delayed idle or balanced idle mode
 * is entered.
 * @param  hhrtim pointer to HAL HRTIM handle
 * @param  TimerIdx Timer index
 *                   This parameter can be one of the following values:
...
__weak void HAL_HRTIM_DelayedProtectionCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Timer_DelayedProtectionCallback could be implemented
     in the user file
   */
}
...
__weak void HAL_HRTIM_CounterResetCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Timer_CounterResetCallback could be implemented in the
     user file
   */
}
...
__weak void HAL_HRTIM_Output1SetCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Timer_Output1SetCallback could be implemented in the
     user file
   */
}
...
__weak void HAL_HRTIM_Output1ResetCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Timer_Output1ResetCallback could be implemented in the
     user file
   */
}
...
__weak void HAL_HRTIM_Output2SetCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Timer_Output2SetCallback could be implemented in the
     user file
   */
}
...
__weak void HAL_HRTIM_Output2ResetCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_Timer_Output2ResetCallback could be implemented in the
     user file
   */
}
...
__weak void HAL_HRTIM_BurstDMATransferCallback(HRTIM_HandleTypeDef *hhrtim,
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_HRTIM_BurstDMATransferCallback could be implemented in the
     user file
   */
}
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_RegisterCallback(HRTIM_HandleTypeDef *hhrtim,
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_UnRegisterCallback(HRTIM_HandleTypeDef *hhrtim,
                             HAL_HRTIM_CallbackIDTypeDef CallbackID) {
  HAL_StatusTypeDef status = HAL_OK;
  ...
  if (HAL_HRTIM_STATE_READY == hhrtim->State) {
    ...
    case HAL_HRTIM_DLLCALBRATIONREADYCALLBACK_CB_ID:
      hhrtim->DLLCalibrationReadyCallback =
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_TIMxRegisterCallback(HRTIM_HandleTypeDef *hhrtim,
                               HAL_HRTIM_CallbackIDTypeDef CallbackID,
 ...
 */
HAL_StatusTypeDef
HAL_HRTIM_TIMxUnRegisterCallback(HRTIM_HandleTypeDef *hhrtim,
                                 HAL_HRTIM_CallbackIDTypeDef CallbackID) {
  HAL_StatusTypeDef status = HAL_OK;
...
HRTIM_TimingUnitBase_Config(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
  ...
  hhrtim->Instance->sTimerxRegs[TimerIdx].REPxR =
      pTimeBaseCfg->RepetitionCounter;
}
 ...
 */
static void
HRTIM_MasterWaveform_Config(HRTIM_HandleTypeDef *hhrtim,
                            const HRTIM_TimerCfgTypeDef *pTimerCfg) {
  uint32_t hrtim_mcr;
  uint32_t hrtim_bmcr;
...
HRTIM_TimingUnitWaveform_Config(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx,
  ...
  */
  if (((pTimerCfg->DelayedProtectionMode !=
...
static void HRTIM_EventConfig(HRTIM_HandleTypeDef *hhrtim, uint32_t Event,
  ...
  case HRTIM_EVENT_1: {
    hrtim_eecr1 &= ~(HRTIM_EECR1_EE1SRC | HRTIM_EECR1_EE1POL |
                     HRTIM_EECR1_EE1SNS | HRTIM_EECR1_EE1FAST);
  ...
  case HRTIM_EVENT_2: {
    hrtim_eecr1 &= ~(HRTIM_EECR1_EE2SRC | HRTIM_EECR1_EE2POL |
                     HRTIM_EECR1_EE2SNS | HRTIM_EECR1_EE2FAST);
  ...
  case HRTIM_EVENT_3: {
    hrtim_eecr1 &= ~(HRTIM_EECR1_EE3SRC | HRTIM_EECR1_EE3POL |
                     HRTIM_EECR1_EE3SNS | HRTIM_EECR1_EE3FAST);
  ...
  case HRTIM_EVENT_4: {
    hrtim_eecr1 &= ~(HRTIM_EECR1_EE4SRC | HRTIM_EECR1_EE4POL |
                     HRTIM_EECR1_EE4SNS | HRTIM_EECR1_EE4FAST);
  ...
  case HRTIM_EVENT_5: {
    hrtim_eecr1 &= ~(HRTIM_EECR1_EE5SRC | HRTIM_EECR1_EE5POL |
                     HRTIM_EECR1_EE5SNS | HRTIM_EECR1_EE5FAST);
  ...
  case HRTIM_EVENT_6: {
    hrtim_eecr2 &=
        ~(HRTIM_EECR2_EE6SRC | HRTIM_EECR2_EE6POL | HRTIM_EECR2_EE6SNS);
  ...
  case HRTIM_EVENT_7: {
    hrtim_eecr2 &=
        ~(HRTIM_EECR2_EE7SRC | HRTIM_EECR2_EE7POL | HRTIM_EECR2_EE7SNS);
  ...
  case HRTIM_EVENT_8: {
    hrtim_eecr2 &=
        ~(HRTIM_EECR2_EE8SRC | HRTIM_EECR2_EE8POL | HRTIM_EECR2_EE8SNS);
  ...
  case HRTIM_EVENT_9: {
    hrtim_eecr2 &=
        ~(HRTIM_EECR2_EE9SRC | HRTIM_EECR2_EE9POL | HRTIM_EECR2_EE9SNS);
  ...
  case HRTIM_EVENT_10: {
    hrtim_eecr2 &=
        ~(HRTIM_EECR2_EE10SRC | HRTIM_EECR2_EE10POL | HRTIM_EECR2_EE10SNS);
...
static void HRTIM_TIM_ResetConfig(HRTIM_HandleTypeDef *hhrtim,
  ...
  case HRTIM_EVENT_10: {
    hhrtim->Instance->sTimerxRegs[TimerIdx].RSTxR =
...
static uint32_t HRTIM_GetITFromOCMode(const HRTIM_HandleTypeDef *hhrtim,
  ...
  case HRTIM_OUTPUT_TE2: {
    ...
    if (((hrtim_set & HRTIM_OUTPUTSET_TIMCMP2) == HRTIM_OUTPUTSET_TIMCMP2) &&
        ((hrtim_reset & HRTIM_OUTPUTRESET_TIMCMP2) ==
         HRTIM_OUTPUTRESET_TIMCMP2)) {
      /* OC mode: HRTIM_BASICOCMODE_TOGGLE */
      interrupt = HRTIM_TIM_IT_CMP2;
    } else if (((hrtim_set & HRTIM_OUTPUTSET_TIMCMP2) ==
...
static void HRTIM_Timer_ISR(HRTIM_HandleTypeDef *hhrtim, uint32_t TimerIdx) {
  uint32_t tisrflags =
      READ_REG(hhrtim->Instance->sTimerxRegs[TimerIdx].TIMxISR);
  uint32_t tdierits =
      READ_REG(hhrtim->Instance->sTimerxRegs[TimerIdx].TIMxDIER);
...
static void HRTIM_DMAMasterCplt(DMA_HandleTypeDef *hdma) {
  HRTIM_HandleTypeDef *hrtim =
      (HRTIM_HandleTypeDef *)((DMA_HandleTypeDef *)hdma)->Parent;

  if ((hrtim->Instance->sMasterRegs.MDIER & HRTIM_MASTER_DMA_MCMP1) !=
      (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
...
#endif /* USE_HAL_HRTIM_REGISTER_CALLBACKS */
  } else if ((hrtim->Instance->sMasterRegs.MDIER & HRTIM_MASTER_DMA_MCMP2) !=
             (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
...
#endif /* USE_HAL_HRTIM_REGISTER_CALLBACKS */
  } else if ((hrtim->Instance->sMasterRegs.MDIER & HRTIM_MASTER_DMA_MCMP3) !=
             (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
...
#endif /* USE_HAL_HRTIM_REGISTER_CALLBACKS */
  } else if ((hrtim->Instance->sMasterRegs.MDIER & HRTIM_MASTER_DMA_SYNC) !=
             (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
...
#endif /* USE_HAL_HRTIM_REGISTER_CALLBACKS */
  } else if ((hrtim->Instance->sMasterRegs.MDIER & HRTIM_MASTER_DMA_MUPD) !=
             (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
...
#endif /* USE_HAL_HRTIM_REGISTER_CALLBACKS */
  } else if ((hrtim->Instance->sMasterRegs.MDIER & HRTIM_MASTER_DMA_MREP) !=
             (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
...
static void HRTIM_DMATimerxCplt(DMA_HandleTypeDef *hdma) {
  ...
  if ((hrtim->Instance->sTimerxRegs[timer_idx].TIMxDIER & HRTIM_TIM_DMA_CMP1) !=
      (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
...
#endif /* USE_HAL_HRTIM_REGISTER_CALLBACKS */
  } else if ((hrtim->Instance->sTimerxRegs[timer_idx].TIMxDIER &
              HRTIM_TIM_DMA_CMP2) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_Compare2EventCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_CMP3) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_Compare3EventCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_CMP4) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_Compare4EventCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_UPD) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_RegistersUpdateCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_CPT1) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_Capture1EventCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_CPT2) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_Capture2EventCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_SET1) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_Output1SetCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_RST1) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_Output1ResetCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_SET2) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_Output2SetCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_RST2) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_Output2ResetCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_RST) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_CounterResetCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_DLYPRT) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
    ...
    HAL_HRTIM_DelayedProtectionCallback(hrtim, timer_idx);
              ...
              HRTIM_TIM_DMA_REP) != (uint32_t)RESET) {
#if (USE_HAL_HRTIM_REGISTER_CALLBACKS == 1)
...
#else
  HAL_HRTIM_BurstDMATransferCallback(hrtim,
                                     GetTimerIdxFromDMAHandle(hrtim, hdma));
#endif /* USE_HAL_HRTIM_REGISTER_CALLBACKS */
}
