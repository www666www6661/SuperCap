  ...
  if (htim == &htim6) // 4kHz tick
  {
    ...
    case 3:
      ...
      if (psData.outputEEnabled) {
        ...
        if (adcData.vWPT < adcData.vB + 0.5f &&
    ...
    if (sysData.systemInited) {
      ...
      if (psData.outputEEnabled) {
        ...
        if (adcData.vCaplf > (CAPARR_MAX_VOLTAGE * 1.01f)) {
          ...
          if (adcData.vCap * capStatus.maxInCurrent >
              adcData.pRefereelf + 120.0f)
        ...
        if (!psData.outputABEnabled || adcData.vWPT < adcData.vB ||
            askData.lowPowerCnt >
                150U) // adcData.pWPT < 3.0f //!psData.outputABEnabled ||
        {
        ...
        if (psData.outputABEnabled &&
            (adcData.vWPT >
             29.55f)) // && sysData.vTick - askData.lastPowerOnTime >
                      // 400U//(adcData.vWPT > adcData.vCap + 0.8f &&
                      // askData.allowRestart) ||
        {
...
static void loop() {
  while (true) {
    HAL_Delay(1);
    WS2812::blink(0, COLOR_BLUE);
    WS2812::blink(1, COLOR_GREEN);
    WS2812::blink(2, COLOR_RED);
  }
}
