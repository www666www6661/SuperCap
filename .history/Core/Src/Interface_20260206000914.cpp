...
void init() {
  ...
  if (HAL_TIMEx_PWMN_Start(&BUZZER_TIM, BUZZER_TIM_CHANNEL) != HAL_OK)
    Error_Handler();
}

void stop() { __HAL_TIM_SET_COMPARE(&BUZZER_TIM, BUZZER_TIM_CHANNEL, 0U); }
    ...
    if (!interfaceStatus.isWarningLast) {
      ...
      case ERROR_UNRECOVERABLE:
        if (errorData.errorCode & ERROR_POWERSTAGE)
          memcpy(interfaceStatus.buzzerNote, buzzerWS_Unrecoverable,
