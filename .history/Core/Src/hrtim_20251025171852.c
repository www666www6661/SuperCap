...
void MX_HRTIM1_Init(void) {
  ...
  if (HAL_HRTIM_ADCTriggerConfig(&hhrtim1, HRTIM_ADCTRIGGER_3,
                                 &pADCTriggerCfg) != HAL_OK) {
    Error_Handler();
  }
  ...
  pCompareCfg.CompareValue = 8000 - 128;
  ...
  if (HAL_HRTIM_DeadTimeConfig(&hhrtim1, HRTIM_TIMERINDEX_TIMER_B,
                               &pDeadTimeCfg) != HAL_OK) {
    Error_Handler();
  }
...
void HAL_HRTIM_MspPostInit(HRTIM_HandleTypeDef *hrtimHandle) {
  ...
  if (hrtimHandle->Instance == HRTIM1) {
    ...
    PA11     ------> HRTIM1_CHB2
    */
    GPIO_InitStruct.Pin = MOS_A_H_Pin | MOS_A_L_Pin | MOS_B_H_Pin | MOS_B_L_Pin;
