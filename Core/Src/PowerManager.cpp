...
__attribute__((section(".code_in_ram"))) void modeStateMachine() {
  ...
  case BOOSTBUCK:
    ...
    if (adcData.vB < (CAPARR_MAX_VOLTAGE * 1.01f))
      __HAL_HRTIM_SETCOMPARE(&hhrtim1, HRTIM_TIMERINDEX_TIMER_B,
                             HRTIM_COMPAREUNIT_3, HRTIM_PERIOD * 0.75f);
    else
      __HAL_HRTIM_SETCOMPARE(
          &hhrtim1, HRTIM_TIMERINDEX_TIMER_B, HRTIM_COMPAREUNIT_3,
          HRTIM_PERIOD * (1 - (adcData.vA * 0.84f) / VB_LIMIT_BY_DUTY));
    // 更新状态和AB占空比
...
__attribute__((section(".code_in_ram"))) void updateADCmf() {
  ...
  adcData.vWPTlf =
      MF_TO_LF_ALPHA * adcData.vWPT + (1.0f - MF_TO_LF_ALPHA) * adcData.vWPTlf;

  adcData.pWPTlf =
      MF_TO_LF_ALPHA * adcData.pWPT + (1.0f - MF_TO_LF_ALPHA) * adcData.pWPTlf;
  ...
  adcData.iCaplf =
      MF_TO_LF_ALPHA * adcData.iCap + (1.0f - MF_TO_LF_ALPHA) * adcData.iCaplf;
  adcData.vCaplf =
      MF_TO_LF_ALPHA * adcData.vCap + (1.0f - MF_TO_LF_ALPHA) * adcData.vCaplf;
...
void powerOnOffControl() {
  if (psData.outputABEnabled) {
    ...
    if (adcData.vA > REFEREE_UVLO_RECOVERY &&
        (errorData.errorLevel == NO_ERROR || errorData.errorLevel == WARNING) &&
...
__attribute__((section(".code_in_ram"))) void updateMaxCurrent() {
  if (adcData.vCap > CAPARR_LOW_VOLTAGE) {
    ...
    capStatus.maxOutCurrent = (CAPARR_MAX_CURRENT - 1.0f) /
